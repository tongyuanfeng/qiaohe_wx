...
def update_userinfo():
    if request.method == 'POST':
        ...
        db = pymysql.connect(host="qiaohequan.com", user="root", passwd="qiaohejiayou", db="qiaohe", port=3306,
                             charset='utf8')
        update_or_insert_userinfo(db, nickname, name, cellphone, weixin_id, level)
        db.close()
        return 'success'
...
def get_user_level_list():
    if request.method == 'POST':
        ...
        cmd = "SELECT nickname,level FROM qiaohe.user "
        cursor.execute(cmd)
        res = cursor.fetchall()
        for line in res:
...
@app.route('/playbook_register/get_userinfo',methods=['GET','POST'])
def get_userinfo():
    if request.method == 'POST':
        gdata = request.get_json(force=True)
        print(gdata)
        ...
        db = pymysql.connect(host="qiaohequan.com", user="root", passwd="qiaohejiayou", db="qiaohe", port=3306,
                             charset='utf8')
        cmd = "insert into qiaohe.playbook_changci(time,place,bookname) values (now(),'{}','{}')".format(place,bookname)
        insert_sql_add(db,cmd)

        cmd = "select * from qiaohe.playbook_changci where bookname='{}' order by id desc limit 1 ".format(bookname)
        playbook_changci= select(db,cmd)
        pid = playbook_changci[0][0]
        for username in users:
            cmd = "select * from qiaohe.user where nickname like '%{}%'".format(username)
            userinfos = select(db,cmd)
            ...
            name = userinfo[3]
            level = userinfo[5]
            cmd = " insert into qiaohe.playbook_player(pid,nickname,user_id,name,level) values('%s','%s','%s','%s','%s')" % (pid,nickname,user_id,name,level)


            insert_sql_add(db,cmd)

        cmd = "select * from qiaohe.playbook_player where pid=%s" % (str(pid))
        print(cmd)
        playbook_players =select(db,cmd)
        db.close()
        return jsonify({"playbook_changci":playbook_changci,"playbook_players":playbook_players})
