...
if __name__ == '__main__':
    db = pymysql.connect(host="qiaohequan.com", user="root", passwd="qiaohejiayou", db="qiaohe",port=3306,charset='utf8')
    ...
    for root, dirs, files in os.walk('newdir'):
        for file in files:
            ...
            file_buf = str.strip('\r\n').strip('\n').strip('\r').strip('  ')
            sql_cmd = "INSERT INTO qiaohe.resume (cellphone,mail,birthday,name,sex,graduate_institutions,profession,record,work_year,file) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (cellphone,mail,birthday,name,sex,graduate_institutions,profession,record,work_year,file_buf)
            insert_sql(db,sql_cmd)
    db.close()